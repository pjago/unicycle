...
(defmethod wheels :omni [_] (s/tuple #{:omni} ::a/R+))
(defmethod wheels :uni  [_] (s/tuple #{:uni}  ::a/R+))
...
(s/def ::params (s/keys :req-un [::kp ::ko ::eps]))
...
(s/def ::omni (s/cat :vx ::a/R  :vy ::a/R :w ::a/R))
...
(defn uni-plan [{:keys [kp ko eps]} [cmd ρ ϕ]]
  (let [D (if (<= ρ eps) 0.0 1.0)
...
(defmethod plan :dff [{:keys [kp ko eps] :as it} task]
...
(defmethod sym :omni [auto [vx vy w]]
  (let [Δyaw      (* w dt)
        Δposition [(* vx dt) (* vy dt) 0]]
    (-> auto
        (update :position + Δposition)
        (update :yaw + Δyaw)
        (update :yaw a/cycle π))))
